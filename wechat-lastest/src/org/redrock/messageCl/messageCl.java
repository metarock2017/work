            ...
            if (Const.EVENT_TYPE_CLICK.equals(eventType)) {
                ...
                if (eventKey.equals("1")) {
                    respContent = "正在创建谁是卧底，请输入游戏人数（4-13之间，不包括法官哦） \n" +
                            "*<a href=\"http://192.168.199.194:8080/myjsp/createwodi.jsp?openner="+toUserName+"\">点击这里自定义词语</a>";
                }
            }
        }
    }
                    ...
                    if ("换".equals(content)) {
                        ...
                        if (content.contains("，")) {
                            ...
                            if (as.length == 2) {
                                ...
                                pstmt1.setInt(4, id);
                                Integer i = DatabaseUtil.executeUpdate(pstmt1);
                                String s = "卧    底：";
    ...
    public void changeWord(Map<String, String> requestMap) {
        ...
        try {
            ...
            if (set.next()) {
                ...
                Long expiretime = System.currentTimeMillis()+30*60*60*1000;
                //update room set wodici=?, pingminci=?, expiretime=? where id=?
                PreparedStatement pstmt2 = con.prepareStatement("update room set wodici=?, pingminci=?, expiretime=? where id=?");
                pstmt2.setString(1, wodici);
                pstmt2.setString(2, pingminci);
                pstmt2.setLong(3, expiretime);
                pstmt2.setInt(4, id2);
                Integer i = DatabaseUtil.executeUpdate(pstmt2);
                if (i != 0) {
                    PreparedStatement pstmt3 = con.prepareStatement("select id from room where openner=?");
                    pstmt3.setString(1, openner);
                    ResultSet set3 = DatabaseUtil.executeQuery(pstmt3);
                    set3.next();
                    Integer id = set3.getInt("id");
                    String s = "卧    底：";
                    Jedis jedis = RedisUtill.getRedisInstance();
